    ...
    <div class="col-md-8 offset-md-2">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><i class="fas fa-plus"></i> Tambah Unit Organisasi</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('unit_organisasi.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('perangkat_daerah_id') is-invalid @enderror" id="perangkat_daerah_id" name="perangkat_daerah_id" required>
                            ...
                            @foreach($perangkatDaerah as $item)
                                <option value="{{ $item->id }}" @selected(old('perangkat_daerah_id') == $item->id)>{{ $item->nama }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('unor_atasan') is-invalid @enderror" id="unor_atasan" name="unor_atasan" value="{{ old('unor_atasan') }}">
                        @error('unor_atasan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
